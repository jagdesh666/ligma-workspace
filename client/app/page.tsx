...
export default function Home() {
  const [shapeAuthors, setShapeAuthors] = useState<Record<string, string>>({})
...
const runExtraction = useCallback(async (ed: any) => {
  ...
  for (const shape of shapes) {
    ...
    result.push({ 
      ...
      text, 
      type: aiData.result, 
      // --- UPDATED: Meta read karne ke bajaye hamari state se author uthao ---
      author: shapeAuthors[shape.id] || role, 
  ...
  setTasks(result)
}, [role, shapeAuthors]) // 👈 Yahan shapeAuthors lazmi add karein taaki data update ho sake
        ...
        <Tldraw autoFocus onMount={handleMount}>
          {editor && (
            <YjsEditorSync
              ...
              shapeAuthors={shapeAuthors} setShapeAuthors={setShapeAuthors}
